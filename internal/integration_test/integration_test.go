...
func TestTestsState(t *testing.T) {
	env := SetupTestEnv(t)
	defer CleanupTestEnv(t, env)

	sess := env.CreateBall(t, "Tests state test ball", session.PriorityMedium)
	store := env.GetStore(t)

	// Initially tests state should be empty
	if sess.TestsState != "" {
		t.Fatalf("Expected empty tests state, got %s", sess.TestsState)
	}

	// Set tests state to needed
	sess.SetTestsState(session.TestsStateNeeded)
	if err := store.UpdateBall(sess); err != nil {
		t.Fatalf("Failed to save session with tests state: %v", err)
	}
	...
	if retrieved.TestsState != session.TestsStateNeeded {
		t.Fatalf("Expected tests state %s, got %s", session.TestsStateNeeded, retrieved.TestsState)
	}

	// Change to done
	retrieved.SetTestsState(session.TestsStateDone)
	if err := store.UpdateBall(retrieved); err != nil {
		t.Fatalf("Failed to update tests state: %v", err)
	}

	// Verify change
	retrieved2, err := store.GetBallByID(sess.ID)
	if err != nil {
		t.Fatalf("Failed to retrieve session: %v", err)
	}
	if retrieved2.TestsState != session.TestsStateDone {
		t.Fatalf("Expected tests state %s, got %s", session.TestsStateDone, retrieved2.TestsState)
	}
}

func TestTestsStateValidation(t *testing.T) {
	testCases := []struct {
		input    string
		expected bool
	}{
		{"", true},          // empty (unset) is valid
		{"not_needed", true},
		{"needed", true},
		{"done", true},
		{"invalid", false},
		{"pending", false},  // not a tests state
		{"complete", false}, // not a tests state
	}

	for _, tc := range testCases {
		result := session.ValidateTestsState(tc.input)
		if result != tc.expected {
			t.Errorf("ValidateTestsState(%q) = %v, expected %v", tc.input, result, tc.expected)
		}
	}
}

func TestTestsStateLabel(t *testing.T) {
	env := SetupTestEnv(t)
	defer CleanupTestEnv(t, env)

	sess := env.CreateBall(t, "Tests state label test", session.PriorityMedium)

	// Test labels
	testCases := []struct {
		state    session.TestsState
		expected string
	}{
		{session.TestsStateUnset, ""},
		{session.TestsStateNotNeeded, "not needed"},
		{session.TestsStateNeeded, "needed"},
		{session.TestsStateDone, "done"},
	}

	for _, tc := range testCases {
		sess.TestsState = tc.state
		if label := sess.TestsStateLabel(); label != tc.expected {
			t.Errorf("TestsStateLabel() for %s = %q, expected %q", tc.state, label, tc.expected)
		}
	}
}

func TestTestsStatePersistence(t *testing.T) {
	env := SetupTestEnv(t)
	defer CleanupTestEnv(t, env)

	store := env.GetStore(t)

	// Create ball with tests state
	sess := env.CreateBall(t, "Tests state persistence", session.PriorityMedium)
	sess.SetTestsState(session.TestsStateNeeded)
	if err := store.UpdateBall(sess); err != nil {
		t.Fatalf("Failed to save session: %v", err)
	}

	// Reload all balls and verify
	...
	for _, ball := range balls {
		if ball.ID == sess.ID {
			found = true
			if ball.TestsState != session.TestsStateNeeded {
				t.Fatalf("Tests state not persisted correctly. Expected %s, got %s",
					session.TestsStateNeeded, ball.TestsState)
			}
		}
	}
	if !found {
		t.Fatalf("Ball %s not found after reload", sess.ID)
	}
}
